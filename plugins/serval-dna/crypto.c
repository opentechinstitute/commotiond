...
static int
serval_create_signature(unsigned char *key,
			...
			const size_t sig_size)
{
  ...
  int sig_ret = crypto_create_signature(key, 
					hash, 
...
static int
serval_extract_sas(unsigned char **key, 
		   int *key_len, 
		   keyring_file *_keyring, 
		   unsigned char *_sid)
{
  // get SAS key associated with our SID
  *key = keyring_find_sas_private(_keyring, _sid, NULL);
...
int
serval_open_keyring(const char *keyring_path,
		    ...
		    keyring_file **_keyring)
{
  ...
  *_keyring = keyring_open(keyring_path_str);
  CHECK_ERR(*_keyring, "Failed to open specified keyring file");
...
int
serval_init_keyring(unsigned char *sid,
		    ...
		    int *key_len)
{
  ...
  if (sid)
    CHECK(sid_len == SID_SIZE, "Invalid SID");
  
  CHECK_ERR(serval_open_keyring(keyring_path, keyring_len, _keyring),
	    "Failed to open keyring");
  
  if (!sid) { //create new sid
    // cycle through the keyring contexts until we find one with room for another identity
    for(int c = 0; c < (*_keyring)->context_count; c++) {
      // create new Serval identity
      new_ident = keyring_create_identity(*_keyring,
					  (*_keyring)->contexts[c],
					  KEYRING_PIN);
      if (new_ident)
	break;
    ...
    CHECK_ERR(keyring_commit(*_keyring) == 0, "Failed to save new SID into keyring");
...
int
cmd_serval_sign(const char *sid_str,
		const size_t sid_len,
		const unsigned char *msg,
		const size_t msg_len,
		char *sig_str_buf,
		...
		const size_t keyring_len)
{
  ...
  if (keyring_path || !sid_len) {
    ...
    CHECK_ERR(serval_extract_sas(&key, NULL, keyring, packedSid),
	      "Failed to fetch SAS key");
  }
  
  CHECK_ERR(serval_create_signature(key,
				    msg,
				    msg_len,
				    signed_msg,
				    SIGNATURE_BYTES + msg_len),
...
error:
  if (_keyring)
    keyring_free(_keyring);
  return ret;
}

int
cmd_serval_verify(const char *sas_key,
		  ...
		  const size_t sig_len)
{
  ...
  int sig_ret = crypto_verify_signature(bin_sas,
					hash,
					crypto_hash_sha512_BYTES,
					&bin_sig[0],
					SIGNATURE_BYTES);
...
int
serval_verify_client(const char *sid_str,
		     const size_t sid_len,
		     const unsigned char *msg,
		     const size_t msg_len,
		     const char *sig,
		     ...
		     const size_t keyring_len)
{
  ...
  int init_ret = serval_init_keyring(packedSid,
				     SID_SIZE,keyring_path,
				     keyring_len,
				     &keyring,
				     NULL,
				     NULL);
  CHECK(init_ret, "Failed to initialize Serval keyring");
  ...
  return cmd_serval_verify(sas_str,
			   2 * SAS_SIZE,
			   msg,
			   msg_len,
			   sig,
...
serval_crypto_handler(co_obj_t *self, co_obj_t **output, co_obj_t *params)
{
  ...
  if (co_str_cmp_str(co_list_element(params, 0), "sign") == 0) {
    ...
    if (list_len == 3) {
      int sign_ret = cmd_serval_sign(_LIST_ELEMENT(params, 1),
				     ...
				     keypath ? co_str_len(co_list_element(params, 3)) - 11 : 0);
      CHECK_ERR(sign_ret, "Failed to create signature");
    } else if (list_len == 2) {
      int sign_ret = cmd_serval_sign(NULL,
				     0,
				     ...
				     keypath ? co_str_len(co_list_element(params, 2)) - 11 : 0);
      CHECK_ERR(sign_ret, "Failed to create signature");
    }
...
//   CHECK(IS_LIST(params) && co_list_length(params) == 2, "Invalid params");
  ...
  int msg_len = co_obj_data((char**)&msg, co_list_element(params, 1));
  int sig_buf_len = SIGNATURE_BYTES + msg_len + 1;
  unsigned char *sig_buf = calloc(sig_buf_len, sizeof(unsigned char));
  
  int sign_ret = serval_create_signature((unsigned char*)_LIST_ELEMENT(params, 0),
					 msg,
					 msg_len,
					 sig_buf,
					 sig_buf_len);
  CHECK(sign_ret, "Failed to sign OLSRd packet");
